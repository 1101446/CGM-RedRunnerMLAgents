...
namespace RedRunner.Characters
{
	...
	public class RedCharacter : Character
	{
		...
		[SerializeField]	//--------
		protected CheckPointCheck m_CheckPointCheck;	//------
		[SerializeField]
		...
		public TrackCheckpoints getTrackCheckpointsRed()	//ritorna la traccia di checkpoint a cui è collegato il personaggio attualmente
		{
		...
		//-----------
		public override CheckPointCheck CheckPointCheck	//con Override mi va in errore, forse manca in qualche altro file?
		{
			get
			{
				return m_CheckPointCheck;
			}
		}
		//-----------
		...
		void Awake ()
		{
			...
			//---------
			m_CheckPointCheck.OnCheckPointed += CheckPointCheck_OnCheckPointed; //aggiungo metodo 'OnCheckpointed'
			//---------
		...
		void Update ()
		{
			...
			// Input Processing
			Move (directionFloat);
			if (jumping == 1)					//effettivo salto effettuato dal metodo Jump(), se nel file RedAgent il parametro viene impostato a 1
			{
				Jump ();
			}
		...
		void LateUpdate ()
		{
			...
			//---------
			m_Animator.SetBool ( "IsCheckPointed", m_CheckPointCheck.IsCheckPointed );
			//---------
...
//					speed = m_WalkSpeed;
		...
		//---------------
		void CheckPointCheck_OnCheckPointed ()		//FORSE NON SERVE PERCHE LO USA IL GROUND SOLO PER IL SALTO E PER IL SUONO 
		{
			if ( !IsDead.Value )
			{
				m_JumpParticleSystem.Play ();
